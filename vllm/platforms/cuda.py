...
def confidential_compute_enabled() -> bool:
    """Query NVML for the GPU confidential-compute (CC) state.

    Returns True when GPU CC mode is active (a Hopper/Blackwell TEE) or any
    protected multi-GPU transport (PPCIE / NVLink-encryption) is in force.
    The result is cached: CC state is fixed for the lifetime of the process.

    Why vLLM cares: under CC every CPU<->GPU copy is routed through an
    encrypted bounce buffer and the encrypt/decrypt is CPU-bound software
    AES-GCM on the issuing core. Pinned host memory therefore provides no
    async-DMA benefit and measurably *hurts* (pinned H2D bandwidth collapses
    to ~0.1-0.2x), so pinning should be disabled. Mirrors TensorRT-LLM's
    ``confidential_compute_enabled()`` (PR #11573).
    """
    from ctypes import byref

    try:
        # Hopper+ exposes a richer settings query (includes multi-GPU mode).
        ...
        # Older driver/GPU: fall back to the simple state query.
        try:
